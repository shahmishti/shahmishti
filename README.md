# 👩‍💻 Mishti Shah

📍 Mumbai, India  
📧 mishti.shah@somaiya.edu  
🔗 [LinkedIn](https://www.linkedin.com/in/mishti-shah-250811310/)

---

## 🎓 Education

**Bachelor of Technology (B.Tech), Computer Engineering**  
University of Mumbai *(2024 – Present)*  

**Higher Secondary Education**  
Wilson College *(2022 – 2024)*  

**ICSE**  
Queen Mary School *(2022)*  

---

## 🚀 Projects

### 🌐 Sabal Avsar – Multilingual Marketplace
**Tech Stack:** HTML, CSS, JavaScript, FastAPI  

- Built a multilingual e-commerce platform to empower rural women by eliminating language barriers (English, Hindi, Marathi, Gujarati)  
- Developed a full-stack decoupled architecture using FastAPI and vanilla JavaScript  
- Implemented seller inventory dashboard and secure JWT-based authentication  
- Designed robust cart and checkout workflows  
- Enabled real-time stock management  

---

### 🤖 PDF-Based Intelligent Question Answering System
**Tech Stack:** Python, Streamlit, LangChain, FAISS, Gemini API  

- Built an AI system to answer natural language queries from PDFs  
- Implemented Retrieval-Augmented Generation (RAG)  
- Used semantic embeddings and FAISS for efficient retrieval  
- Developed an interactive interface using Streamlit  

---

## 🧠 Training Experience

### Gen-AI & Agentic AI *(Dec 2025 – Jan 2026)*  

- Developed generative AI applications using LLMs  
- Designed agentic AI workflows with autonomous decision-making  
- Integrated APIs and applied prompt engineering  

---

## 📜 Certifications

- **IBM SkillsBuild – Student Ambassador Program**  
  - Built a retrieval-supported system improving verification processes by 45%  

- **Responsive Web Design – freeCodeCamp**  
  - Built responsive web pages using HTML and CSS  

- **Augmented Reality – BharatXR**  
  - Created AR/VR Snapchat lenses  

---

## 🛠️ Technical Skills

### 💻 Programming
- Python, Java, C, C++  
- HTML, CSS, JavaScript  

### ⚙️ Frameworks & Tools
- FastAPI  
- Streamlit  
- FAISS  
- MySQL  
- Excel (Advanced)  

### 🔧 Engineering Skills
- Design verification  
- Test execution  
- Defect analysis  
- Process mapping  

### 🤝 Soft Skills
- Team collaboration  
- Report writing  
- Requirement analysis  
- Communication  

---

## 🌟 About Me

Computer Engineering student passionate about AI, web development, and building impactful tech solutions. Interested in multilingual systems, accessibility, and intelligent automation.

---

## 📫 Contact

📧 mishti.shah@somaiya.edu  
🔗 [LinkedIn](https://www.linkedin.com/in/mishti-shah-250811310/)
